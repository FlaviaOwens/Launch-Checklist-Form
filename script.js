window.addEventListener("load", function() {
	...
	form.addEventListener("submit", function(event) {
		...
		if (pilotName === "" || copilotName === "" || fuelLevel === '' || isNaN(fuelLevel) || cargoMass === '' || isNaN(cargoMass) ) {
			...
			launchStatus.innerHTML = 'Information Required Before Launch';

		} else {

			items.style.visibility = 'visible';

			document.getElementById('pilotStatus').innerHTML = `Pilot ${ pilotName + ' ' }Ready Check Complete`
			document.getElementById('copilotStatus').innerHTML = `Co-pilot ${ copilotName + ' ' }Ready Check Complete`
			...
			if (cargoMass > 10000) {
				ready = false;
				cargoStatus.innerHTML = 'Cargo Mass exceeds launch maximum';
			} else {
